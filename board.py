...
class Board:
    ...
    def draw_grid(self):
        ...
        for i in range(1,3):
            p.draw.rect(self.screen, "black", p.Rect(0, (i*self.SCREEN_HEIGHT/3) - (outer_cell_line_width / 2), self.SCREEN_WIDTH, outer_cell_line_width))
        ...
        for i in range(1,9):
            p.draw.rect(self.screen, "black", p.Rect(0, (i*self.SCREEN_HEIGHT/9) - (inner_cell_line_width / 2), self.SCREEN_WIDTH, inner_cell_line_width))
    ...
    def draw_number(self,num, row, col):
        # set up number
        number_size = round(self.SCREEN_HEIGHT/18)
